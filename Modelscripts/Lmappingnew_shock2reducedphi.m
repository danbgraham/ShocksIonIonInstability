...
Tiw = 12; % eV. Temperature used for weighted particles. 
Tio = 3; % eV. Temperature of the distribution being modelled. 

vithw = sqrt(2*e*Tiw/mi);
vitho = sqrt(2*e*Tio/mi);
...
Ex = (-Jz.*By./(e*ni)+P0*sech(xvec/l).^2./(e*ni*l))*0.9;
Ey = (Jz.*Bx./(e*ni))*0.9;
Ez = -Vsw.*(B1-B0);
...
xrange = [-2000 500]*1e3;
dxx = 10e3;
vxrange = [-1400 1400]*1e3;
vyrange = [-1400 1400]*1e3;
vzrange = [-1400 1400]*1e3;
dv = 10e3;
...
histstruct.n1Dvx = n1Dvx/integraldx*nsw;
histstruct.n1Dvy = n1Dvy/integraldx*nsw;
histstruct.n1Dvz = n1Dvz/integraldx*nsw;

integraldxdy = sum(sum(n2Dvxvy(end,:,:)))*dv^2;
histstruct.n2Dvxvy = n2Dvxvy/integraldxdy*nsw;
histstruct.n2Dvxvz = n2Dvxvz/integraldxdy*nsw;
histstruct.n2Dvyvz = n2Dvyvz/integraldxdy*nsw;
...
save('histstruct_shock2reducedphi.mat','histstruct')
...
Ex = (-Jz.*By./(e*ni)+P0*sech(q(1)/l).^2./(e*ni*l))*0.9;
Ey = (Jz.*Bx./(e*ni))*0.9;
Ez = -vsw.*(B1-B0);
